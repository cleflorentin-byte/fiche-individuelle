  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === "signup") {
        ...
        setInfo(
          "Compte créé. Ton accès doit être validé manuellement par ton délégué CFDT avant de pouvoir consulter tes données."
        );
      } else {
    ...
    <div style={{ background: CREAM, minHeight: "100vh" }} className="flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div style={{ background: SLATE, borderTop: `5px solid ${ORANGE}` }} className="rounded-t-lg px-6 py-5">
          ...
          <h1 className="board-font text-xl font-bold uppercase tracking-wide text-white">
            Suivi des compteurs réglementaires
          </h1>
        </div>

        <form onSubmit={handleSubmit} className="bg-white rounded-b-lg p-6 flex flex-col gap-3" style={{ border: "1px solid #DCD5C5" }}>
          <div className="flex gap-2 mb-2">
            <button type="button" onClick={() => setMode("signin")} className="flex-1 text-sm font-medium py-1.5 rounded-md"
              style={mode === "signin" ? { background: SLATE, color: "white" } : { background: CREAM, color: INK }}>
              Connexion
            </button>
            <button type="button" onClick={() => setMode("signup")} className="flex-1 text-sm font-medium py-1.5 rounded-md"
          ...
          {mode === "signup" && (
            <label className="text-xs block">
              ...
              <input value={fullName} onChange={(e) => setFullName(e.target.value)} required
                className="w-full text-sm rounded-md px-3 py-2" style={{ border: "1px solid #DCD5C5" }} />
            </label>
          )}
          ...
          <button type="submit" disabled={busy} className="mt-2 text-sm font-medium py-2 rounded-md"
